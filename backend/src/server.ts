...
const MERKLE_HASHER_DIR = path.resolve(__dirname, '../../circuits/merkle-hasher')
const WITHDRAWAL_DIR = path.resolve(__dirname, '../../circuits/withdrawal')
const SUNSPOT_BIN = 'sunspot'
...
async function getNextLeafIndex(): Promise<number> {
  try {
    ...
    const accountInfo = await rpc.getAccountInfo(address(poolPda), { encoding: 'base64' }).send()
...
function generateProof(): { proof: Buffer, publicWitness: Buffer } {
  ...
  execSync(`${SUNSPOT_BIN} prove target/withdrawal.json target/withdrawal.gz target/withdrawal.ccs target/withdrawal.pk`, {
    cwd: WITHDRAWAL_DIR,
    stdio: 'pipe'
  })

  const proof = fs.readFileSync(path.join(WITHDRAWAL_DIR, 'target/withdrawal.proof'))
  const publicWitness = fs.readFileSync(path.join(WITHDRAWAL_DIR, 'target/withdrawal.pw'))
...
app.post('/api/deposit', async (req, res) => {
  try {
    ...
    if (amountNum < MIN_DEPOSIT) {
      return res.status(400).json({ error: `Invalid amount: minimum deposit is ${MIN_DEPOSIT} lamports (0.001 SOL)` })
    }

    const leafIndex = await getNextLeafIndex()
    console.log(`Generating deposit for ${amount} lamports at leaf index ${leafIndex}...`)
...
app.post('/api/withdraw', (req, res) => {
  try {
    ...
    const requiredFields = ['nullifier', 'secret', 'amount', 'commitment', 'nullifierHash', 'merkleRoot', 'leafIndex']
...
app.post('/api/airdrop', async (req, res) => {
  try {
    ...
    console.log(`Requesting airdrop of ${Number(lamports) / Number(LAMPORTS_PER_SOL)} SOL to ${addressStr}...`)

    const airdropSignature = await rpc.requestAirdrop(recipient, lamports).send()
